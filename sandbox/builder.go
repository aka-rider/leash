...
`)
	fmt.Fprintf(&sb, `
;; User keychain (READ-ONLY): OAuth/credential-based CLI tools (claude,
;; docker's osxkeychain credential helper, etc.) authenticate via the user's
;; login keychain. The actual add/update mutation is mediated by securityd
;; over Mach IPC (already unconditionally allowed below); this only grants
;; the client read access to the keychain files themselves.
(allow file-read*
  (subpath %q))
`, b.home+"/Library/Keychains")
	fmt.Fprintf(&sb, `
;; Tmp (read + write)
...
// file-read* is implied by every permission level; Write and Exec bits each
// add their own ops on top, so a combined Write|Exec grant keeps both
// file-write* and the exec ops instead of one silently shadowing the other.
func sbplOps(p Permission) string {
	...
	if p&Exec != 0 {
		ops += " file-map-executable process-exec"
	}
	return ops
}

// sbplDenyOps returns the SBPL operation string for a deny rule.
// Deny rules are targeted per bit — -r denies reads, -w denies writes, -x
// denies exec — and a combined deny (e.g. Write|Exec) denies the union of
// each bit's ops rather than only the last one checked.
func sbplDenyOps(p Permission) string {
	var ops []string
	if p&Read != 0 {
		ops = append(ops, "file-read*")
	}
	if p&Write != 0 {
		ops = append(ops, "file-write*")
	}
	if p&Exec != 0 {
		ops = append(ops, "file-map-executable process-exec")
	}
	if len(ops) == 0 {
		return "file-read*"
	}
