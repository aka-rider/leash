...
- **`detect`** — one file per tool (claude, docker, git, go, homebrew, npm, python, rust, xcode). Every detector is probed unconditionally on each run from `execute_darwin.go`; a missing binary is a silent no-op, any other failure is fatal.
