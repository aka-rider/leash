...
- **`detect/golang.go` is the closest analog:** stdlib-only, `//go:build darwin`, `exec.LookPath` → `errors.Is(err, exec.ErrNotFound)` early no-op → `filepath.EvalSymlinks` → `p.Allow(filepath.Dir(bin), sandbox.Exec)` → `AllowOptional` for caches. Detectors that shell out use `probeOutput` (`detect/probe.go`, Setsid). Rust needs no shell-out.
...
- **`AllowOptional` (`sandbox/profile.go:89-97`)** skips `os.ErrNotExist`, propagates other errors — use for `~/.cargo`, `~/.rustup`.
...
- **Hermetic test writing to real `~/.cargo`:** avoided by pointing `CARGO_HOME` at a writable subdir of the temp crate via `Config.ExtraEnv` and setting `CARGO_NET_OFFLINE=1` (WP4), so the test never touches the real home or the network.
...
3. Grant the toolchain caches: `p.AllowOptional("~/.cargo", sandbox.Write)` (gap #1, CARGO_HOME registry writes) and `p.AllowOptional("~/.rustup", sandbox.Exec)` (decision #3, rustup toolchain binaries + std read); wrap each error.
...
5. Construct `sandbox.Config{Profiles: []sandbox.Snapshot{grant.Snapshot(), p.Snapshot()}, ExtraEnv: []string{"CARGO_HOME=" + filepath.Join(crateDir, "cargohome"), "CARGO_NET_OFFLINE=1"}}`; run `exec.Command(cargoBin, "test")` with `cmd.Dir = crateDir`, capturing both stdout and stderr buffers inline (do not reuse `sandboxRun`, which discards stderr); use a 120 s `context.WithTimeout`.
