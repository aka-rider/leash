...
func TestOpenTraceSink_EmptyPath(t *testing.T) {
	// Empty path resolves to ./sbx-trace.log; we just check no error is returned
	// without changing the working directory (the file lands in the test's temp dir via chdir).
	dir := t.TempDir()
	orig, err := os.Getwd()
	if err != nil {
		t.Fatal(err)
	...
	defer f.Close()
	if _, err := os.Stat(filepath.Join(dir, "sbx-trace.log")); err != nil {
		t.Fatalf("expected ./sbx-trace.log, stat failed: %v", err)
	}
}
