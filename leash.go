...
type Leash struct {
	...
	Reads   []string
	Writes  []string
	// FutureWrites grants write on exact paths that may not exist yet
	// (created by the child inside the sandbox); used by --worktree for
	// git's packed-refs lock/tempfiles.
	FutureWrites []string
	Execs        []string
	DenyReads    []string
	DenyWrites   []string
