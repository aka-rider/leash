...
func Homebrew(p sandbox.ToolProfile) (sandbox.ToolProfile, error) {
	...
	prefix, err := probeOutput(exec.Command(brewPath, "--prefix"))
	if err != nil {
	...
	// {prefix}/bin entries are symlinks into Cellar; Seatbelt's process-exec
	// check resolves symlinks first, so the real binaries under Cellar also
	// need an exec grant (read on the whole prefix is already granted above).
	if _, err := p.AllowOptional(filepath.Join(basePath, "Cellar"), sandbox.Exec); err != nil {
		return p, fmt.Errorf("detect homebrew cellar exec: %w", err)
	}
